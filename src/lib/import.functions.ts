...
export const importProduct = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ImportResult> => {
    ...
    try {
      ...
      if (!res.ok) {
        return { ok: false, fallbackToManual: true, error: `Source returned ${res.status}. Fill manually.` };
      }
      html = await res.text();
    } catch (e) {
    ...
    const og = (k: string) => $(`meta[property="og:${k}"]`).attr("content") || $(`meta[name="og:${k}"]`).attr("content") || "";

    const j = jsonProduct as Record<string, any> | null;
    const title = (j?.name as string) || og("title") || $("h1").first().text().trim() || "";
    const brand = (typeof j?.brand === "object" ? j?.brand?.name : j?.brand) || "";
    const description = (j?.description as string) || og("description") || $('meta[name="description"]').attr("content") || "";
    const offers = j?.offers ? (Array.isArray(j.offers) ? j.offers[0] : j.offers) : null;
    const priceRaw = offers?.price || offers?.lowPrice || $('[data-price],[itemprop="price"]').first().attr("content") || "";
    const sourcePrice = priceRaw ? parseFloat(String(priceRaw).replace(/[^\d.]/g, "")) : null;
    const ratingRaw = j?.aggregateRating?.ratingValue;
    const availability = offers?.availability ? String(offers.availability).split("/").pop() ?? "" : "";
    ...
    if (!title) {
      return { ok: false, fallbackToManual: true, error: "Could not parse product. Fill manually." };
    }
