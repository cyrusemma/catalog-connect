...
function AdminProductsPage() {
  ...
  const togglePublish = (p: Product) => {
    ...
    toast.success(!p.is_published ? "Published" : "Unpublished");
  };

  const toggleFeature = (p: Product) => {
    ...
    <div className="space-y-4 pb-20 md:pb-0">
      <div className="flex items-center justify-between">
        ...
        <Link to="/admin/products/new">
          <Button className="rounded-full raised">Add product</Button>
        </Link>
      </div>
      ...
      {loading ? (
                ...
                return (
                  <tr key={p.id} className="border-t border-border">
                    ...
                    <td className="hidden p-3 md:table-cell">
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs ${p.is_published ? "bg-success/20 text-success" : "bg-muted text-muted-foreground"}`}
                      >
                    ...
                    <td className="p-3 text-right">
                      <div className="inline-flex flex-wrap items-center justify-end gap-1">
                        ...
                        >
                          <Star
                            className={`size-4 ${p.is_featured ? "fill-primary text-primary" : ""}`}
                          />
                        </Button>
                        <Button
                          variant="ghost"
                        ...
                        <a
                          href={buildWhatsAppShareUrl(
                            `${p.title} — ${formatMoney(p.selling_price, settings.currency)}\n${url}`,
                          )}
                          target="_blank"
        ...
        <div className="rounded-2xl bg-card p-10 text-center text-muted-foreground raised">
          No products yet.{" "}
          <Link to="/admin/products/new" className="text-primary underline">
