    ...
    >
      ...
      <section className="rounded-2xl bg-card p-5 neu-raised">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <Input
              ...
              type="number"
              step="0.01"
              value={value.source_price ?? ""}
              onChange={(e) =>
                set("source_price", e.target.value ? parseFloat(e.target.value) : null)
              }
            />
          </div>
          <div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            {[1.2, 1.5, 1.8, 2].map((m) => (
              <button
                key={m}
                type="button"
                onClick={() => setSell(sourcePrice * m)}
                className="rounded-full bg-secondary px-3 py-1 text-xs font-medium neu-button hover:text-primary"
              >
                ×{m}
              </button>
            ))}
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-xl bg-background p-3 neu-pressed">
            ...
            <Input
              className="mt-0.5 h-7 rounded-lg border-0 bg-transparent p-0 text-lg font-bold focus-visible:ring-0"
              type="number"
              value={value.stock}
      ...
      <section className="rounded-2xl bg-card p-5 neu-raised">
        ...
        <div className="mb-3 grid grid-cols-3 gap-2 sm:grid-cols-5">
          {value.images.map((img, i) => (
            <div
              key={i}
              className="relative aspect-square overflow-hidden rounded-xl bg-muted neu-button"
            >
              ...
              <button
                ...
                className="absolute right-1 top-1 grid size-6 place-items-center rounded-full bg-background/90 hover:bg-destructive hover:text-destructive-foreground"
              >
                <X className="size-3" />
              </button>
            </div>
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            onClick={() => {
              if (imgUrl) {
                set("images", [...value.images, imgUrl]);
                setImgUrl("");
              }
            }}
          >
      ...
      <section className="rounded-2xl bg-card p-5 neu-raised">
        ...
        <ul className="mb-3 space-y-1.5 text-sm">
          {Object.entries(value.specifications).map(([k, v]) => (
            <li
              key={k}
              className="flex items-center justify-between rounded-xl bg-muted/50 px-3 py-1.5"
            >
              <span>
              ...
              <button
                type="button"
                onClick={() => {
                  const c = { ...value.specifications };
                  delete c[k];
                  set("specifications", c);
                }}
              >
                <X className="size-3" />
              </button>
        ...
        <div className="grid grid-cols-[1fr_1fr_auto] gap-2">
          ...
          <Button
            ...
            onClick={() => {
              if (specKey && specVal) {
                set("specifications", { ...value.specifications, [specKey]: specVal });
                setSpecKey("");
                setSpecVal("");
              }
            }}
      ...
      <section className="rounded-2xl bg-card p-5 neu-raised">
        ...
        <div className="flex flex-wrap gap-6">
          ...
          <label className="flex items-center gap-2 text-sm font-medium">
            <Switch checked={value.is_featured} onCheckedChange={(v) => set("is_featured", v)} />{" "}
            Featured
          </label>
        </div>
      </section>
      ...
      <Button
        ...
        disabled={busy}
        size="lg"
        className="w-full rounded-2xl neu-button bg-primary py-6 text-base font-semibold text-primary-foreground hover:bg-primary"
      >
